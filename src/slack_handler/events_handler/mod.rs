...
use crate::{
    aws::event_bridge_scheduler::EventBridgeScheduler, config::Config,
    db::dynamodb::{ScheduledTasksDynamodb, SlackInstallationsDynamoDb},
    errors::AppError,
    slack_handler::{events_handler::{app_home_opened_handler::AppHomeOpenedEvent, slack_request::parse_slack_request}, utils::{request_utils::validate_request, slack_response::response}}};
...
pub async fn handle_slack_events(config: &Arc<Config>, event: ApiGatewayProxyRequest) -> Result<ApiGatewayProxyResponse, AppError> {
    ...
    match slack_request {
        ...
        SlackPushEvent::EventCallback(callback_event) => {
            match &callback_event.event {
                SlackEventCallbackBody::AppHomeOpened(app_home_opened_event) => {
                    json_tracing::info!(
                        ...
                        enterprise_id = &callback_event.enterprise_id.as_ref().map(|e| e.0.as_str()).unwrap_or(""),
                    );

                    let home_opened_event = AppHomeOpenedEvent {
                        user_id: app_home_opened_event.user.0.clone(),
                        team_id: callback_event.team_id.0.clone(),
                        enterprise_id: callback_event.enterprise_id.as_ref().map(|e| e.0.clone()).unwrap_or_default(),
                    };
