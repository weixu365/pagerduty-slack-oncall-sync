...
pub async fn handle_slack_interactive_async(config: &Arc<Config>, event: ApiGatewayProxyRequest) -> Result<ApiGatewayProxyResponse, AppError> {
    ...
    if let Some(action) = payload.actions.first() {
        let slack_view = match action.action_id.as_str() {
            "delete_schedule" => handle_delete_schedule(&payload, action, &scheduled_tasks_db, next_trigger_timestamp).await,
            "refresh" => handle_refresh(&payload, action, &scheduled_tasks_db, next_trigger_timestamp).await,
            "filter_select" => handle_filter_change(&payload, action, &scheduled_tasks_db, next_trigger_timestamp).await,
            "page_size_select" => handle_page_size_change(&payload, action, &scheduled_tasks_db, next_trigger_timestamp).await,
            "page_previous" | "page_next" => handle_pagination(&payload, action, &scheduled_tasks_db, next_trigger_timestamp).await,
